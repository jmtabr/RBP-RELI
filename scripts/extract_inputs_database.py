...
The background BED is expanded to per-base positions for RELI's permutation
null model using build_null_model.py.
...
from reli_utils import STANDARD_CHROMS
...
def main():
    ...
    print(f"\nDone. Next steps:")
    print(f"  1. Build null model:  python scripts/build_null_model.py --input-dir {args.output_dir} --genome-build data/GenomeBuild/hg19.txt")
    print(f"  2. Run GPU-RELI (single query mode):")
    print(f"     python -m reli_turbo -snp {snp_path} -null {args.output_dir}/Null_Model_BG_{args.prefix} \\")
    print(f"       -out <output_dir> -phenotype {args.prefix} -index <CLIPseq.index> \\")
    print(f"       -data <peaks_dir> -build <hg19.txt> -rep 2000 -corr <N>")
    print(f"  Note: Do NOT use --per-target-files. The default produces consolidated TSVs.")
