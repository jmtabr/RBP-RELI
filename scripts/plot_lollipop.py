#!/usr/bin/env python3
"""
RBP-RELI lollipop plot: NAR-style layout with horizontal exon schematic.
...
# ---------------------------------------------------------------------------
# rcParams: journal standards (Arial/Helvetica, no legend frame)
# ---------------------------------------------------------------------------
...
def draw_vertical_lollipop(ax, df, cmap, edge_color, direction, y_max, top_n):
    ...
    if direction == "INCL":
        ...
        ax.set_xticklabels(rbp_names, rotation=55, ha="right", fontsize=FS_TICK)
        ...
        ax.set_xticklabels(rbp_names, rotation=55, ha="left", fontsize=FS_TICK,
...
def main():
    ...
    if splice_files and not deseq2_files:
        ...
        mode = "splice"  # default if both present
    else:
