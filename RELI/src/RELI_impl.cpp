...
void RELI::overlapping2(vector<SNP> SNPvecA, vector<bed3col> bedvecA){
	...
	for (vector<SNP>::iterator snpit = tempsnpvec.begin(); snpit != tempsnpvec.end(); ++snpit){
		// Skip SNPs on chromosomes not present in the target BED file
		if (targetbedfileindex_start.find(snpit->snp_chr) == targetbedfileindex_start.end())
			continue;
...
void RELI::target_bed_file::readingData(string inStr, bool inVal){
	...
	while (!in.eof()){
		...
		// Skip chromosomes not in the genome build file (e.g., random contigs,
		// unplaced scaffolds like chr4_gl000194_random, chrUn_gl000218).
		// Matches GPU backend behavior (io.py: "if chrom not in chrom_to_idx: continue").
		// Without this check, peaks on unknown chromosomes cause segfaults in
		// the overlap scanning functions.
		{
			bool chrom_known = false;
			for (auto &cs : RELI::chromosome_strucuture) {
				if (cs.first == newTab.bed_chr) { chrom_known = true; break; }
			}
			if (!chrom_known) continue;
		}
		buffer = buffer.substr(buffer.find_first_of("\t") + 1, bufferSize);
...
void RELI::RELIobj::public_ver_set_target_data(){ 
	...
	if (k != this->dataindexvec.end()){
		...
		cerr << "cannot find corresponding data entry in the index file, exiting." << endl;
		exit(-1);
	}
...
void RELI::MAF_binned_null_model::loading_null_data(string rhs){
	...
	while (!in.eof()){
		...
		buffer = bufferChar;
		if (RELI::snp_matching){ 
			RELI::binned_null_model_data.bin_map[strtoul(linehandler(buffer).at(1).c_str(), NULL, 10)]->push_back(strtoul(linehandler(buffer).at(0).c_str(), NULL, 10));
		}
		else{
			RELI::binned_null_model_data.bin0.push_back(strtoul(linehandler(buffer).at(0).c_str(), NULL, 10));
		}
	}
	in.close();
